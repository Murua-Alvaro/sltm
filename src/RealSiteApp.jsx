...
export default function RealSiteApp(){
 ...
 const setDelimitation=m=>{stopWatch();setMode(m);setUserPoly([]);setWalkTrack([]);setWalking(false)};
 const addCurrentCorner=async()=>{let g=gps;if(!g)g=await locate();if(g)setUserPoly(x=>[...x,[g.lat,g.lng]])};
 const closeWalk=()=>{if(walkTrack.length>=3){const simplified=[];walkTrack.forEach(p=>{const q=[p.lat,p.lng];if(!simplified.length||hav({lat:simplified.at(-1)[0],lng:simplified.at(-1)[1]},p)>=4)simplified.push(q)});setUserPoly(simplified);setWalking(false);stopWatch()}};
